...
static inline void sock_ops_establish_cb(struct bpf_sock_ops *skops, u8 sock_type) {
	int err;
...
static inline void sock_ops_rtt_cb(struct bpf_sock_ops *skops) {
	struct sk_key sk_key = {};
	struct sk_info *sk_info;
...
static inline void sock_ops_state_cb(struct bpf_sock_ops *skops) {
	struct sk_key sk_key = {};
...
int bpf_sockops_cb(struct bpf_sock_ops *skops) {
	...
	case BPF_SOCK_OPS_ACTIVE_ESTABLISHED_CB:
		sock_ops_establish_cb(skops, SOCK_TYPE_ACTIVE);
		break;
	case BPF_SOCK_OPS_PASSIVE_ESTABLISHED_CB:
		sock_ops_establish_cb(skops, SOCK_TYPE_PASSIVE);
		break;
	case BPF_SOCK_OPS_RTT_CB:
		sock_ops_rtt_cb(skops);
		break;
	case BPF_SOCK_OPS_STATE_CB:
		sock_ops_state_cb(skops);
		break;
	}
